...
- **macOS**(原生 InputMethodKit / Swift):`cd desktop/macos && ./build.sh install`
  ...
  (逐字对齐),菜单可切 5 种文字,光标旁有词义/词根浮窗 + 完整 DPD 词典,且支持
  类中文输入法的**候选窗**(数字选词、`-`/`=` 翻页)与**下一词预测**。
- **iOS / iPadOS**(原生键盘扩展 / Swift):复用同一引擎,用 Xcode 建工程打包;见
  [`desktop/ios/`](desktop/ios/)。
- **Android**(原生 InputMethodService / Kotlin):引擎移植到 Kotlin(JVM 测试通过),
...
最新发行版([Releases](https://github.com/juncoflockleader/pali-input/releases/latest))附带可直接安装的包:

- **macOS** — `PaliIME-macOS-v1.1.0.zip`(ad-hoc 签名)。解压得 `PaliIME.app`,放进
  `~/Library/Input Methods/`,在「系统设置 → 键盘 → 输入法」添加「Pali」。首次启用若遇
...
针对你**正在输入的那个词**，实时给出多类提示（见 [predict.js](predict.js)）：
...
2. **补全 Completion**——按三藏词频排序的**整词补全**;**下一词 Next word**——按
   巴利圣典语料的 bigram 模型预测**下一个词**(点选追加,可连续接龙)。
3. **可能的词根 √dhātu**——匹配的[巴利词根](roots.js),含英/中释义;
   也能通过现在式词干匹配(如输入 `pass` → √dis「见」),并**自动剥离前缀**
   (如 `anubudh` → upasagga `anu-` + √budh)。点一下填入该词。
4. **可能的前缀 upasagga**——20 个传统动词前缀,正在键入或已键入的都会列出。
5. **拆分 Analysis**——对正在输入的**整词**做形态拆解:
...
└── desktop/     桌面 / 移动端原生输入法（macOS / iOS / Android / Keyman）
```
...
**词根**来自 **[Digital Pāḷi Dictionary (dpd-db)](https://github.com/digitalpalidictionary/dpd-db)**
...
以及词库、形态拆解、补全、下一词预测。各原生端引擎(Swift / Kotlin)由 CI 另行测试。

## 已知限制 · Known limitations

- **缅甸文**:ṅ 作为连写首音的 *kinzi*(င်္,如 saṅgha→သင်္ဃ)已正确处理。
- 引擎面向标准巴利音系；不处理梵语特有音（ṛ ṝ ḹ ś ṣ ḥ 等）。
...
| **macOS** | Release 附 `.app`(ad-hoc 签名)。**自用**:首次在「系统设置 → 隐私与安全性」点「仍要打开」即可。**公开分发零警告**:用 Developer ID 签名 + 公证(notarize),用户双击即开 | 自用不需要;签名 + 公证需 Apple 开发者账号 |
| **iOS / iPadOS** | Xcode 建工程,真机 / 模拟器运行;对外走 App Store 或 TestFlight | 需 Apple 开发者账号 |
| **Android** | Release 附 `.apk`,可直接发给人 sideload;或上架 Google Play | sideload 不需要;上架 Play 需开发者账号 |
...
- **下一词语料**([bilara-data](https://github.com/suttacentral/bilara-data))为 **CC0**。
